...
where T: Copy + Default + Float + ScalarOperand + 'static+ std::fmt::Debug{
    ...
    while i< ags.m{
        ...
        for k in 0..i{
            let (dx, dy)=apply_plane_rotation(ags.H[(k, i)], ags.H[(k+1, i)], ags.cs[k], ags.sn[k]);
            ags.H[(k, i)]=dx;
            ags.H[(k+1, i)]=dy;
        }
        ...
        ags.sn[i]=sn1;
        {
            let (dx, dy)=apply_plane_rotation(ags.H[(i, i)], ags.H[(i+1, i)], ags.cs[i], ags.sn[i]);
            ags.H[(i, i)]=dx;
            ags.H[(i+1, i)]=dy;
        }
        {
...
where T: Copy + Default + Float + ScalarOperand + 'static+ std::fmt::Debug
{
    pub fn create(problem_size: usize, m: usize, m_max: usize, m_min: usize, m_step: usize, cf: T, tol: T)->AGmresState<T>{
        AGmresState{
            m,
            m_max,
            m_min, 
    ...
    pub fn init(&mut self, A: &dyn Fn(ArrayView1<T>)->Array1<T>, x: &Array1<T>, b: ArrayView1<T>,M: &dyn Fn(ArrayView1<T>)->Array1<T>){
        let mut w=M(b.view());
        let normb={
            let nb=norm(&w.view());
            if nb==T::zero(){
    ...
    pub fn next(&mut self, A: &dyn Fn(ArrayView1<T>)->Array1<T>, x: &mut Array1<T>, b: ArrayView1<T>,M: &dyn Fn(ArrayView1<T>)->Array1<T>){
        if self.converged{
            return;
        }
