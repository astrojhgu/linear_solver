    ...
    T: Copy + Default + Float + ScalarOperand + 'static + std::fmt::Debug,
{
    ...
    for j in 0..=m {
        ...
        if Hess[(j + 1, j)] == T::zero() || j == m {
            ...
            return (
                Hess.slice(s![0..=j + 1, 0..=j]).to_owned(),
                Vm.slice(s![.., 0..=j]).to_owned(),
            );
        }
        Vm.column_mut(j + 1).assign(&(w / Hess[(j + 1, j)]));
    }
    (
        Hess.slice(s![0..=m + 1, 0..=m]).to_owned(),
        Vm.slice(s![.., 0..=m]).to_owned(),
    )
}
...
where
    T: Copy + Default + Float + ScalarOperand + 'static + std::fmt::Debug,
{
    let restart = if restart > b.len() { b.len() } else { restart };
    assert!(b.len() == x.len());
    let n = b.len();
